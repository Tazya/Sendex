...
class Sendex {
	...
	public function sendEmail($email, array $options = array()) {
		...
		$mail->set(modMail::MAIL_FROM, $this->modx->getOption('email_from', $options, $this->modx->getOptions('emailsender'), true));
		...
		$response = !$mail->send() ? $mail->mailer->errorInfo : $mail->reset();
